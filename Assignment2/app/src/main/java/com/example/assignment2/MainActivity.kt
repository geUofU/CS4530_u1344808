...
class MyViewModel : ViewModel(){
    private val courses = MutableStateFlow(listOf<Map<String, String>>())
    val coursesReadOnly : StateFlow<List<Map<String, String>>> = courses

    // Add a course to the list
    fun addCourse(inputMap: Map<String, String>){
        if(!courses.value.contains(inputMap)){
            if(inputMap["dep"] != null && inputMap["num"] != null && inputMap["loc"] != null){
                courses.value = courses.value + inputMap
            }
        }
    }
    ...
    fun removeCourse(inputMap: Map<String, String>){
        courses.value = courses.value.filter{it != inputMap}
    }
}
...
fun CourseList(vm: MyViewModel){
    ...
    {
        val observableList by vm.coursesReadOnly.collectAsState()
        ...
        Row{
            LazyColumn{
                items(observableList) { Text("${it["dep"]} ${it["num"]}",
                    fontSize = 20.sp,
